...
export default function ViewProduct() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        const r = await fetch(`${getApiBase()}/auth/me.php`, {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        setLoading(true);
        setError(null);
        const r = await fetch(`${getApiBase()}/items/view.php?product_id=${encodeURIComponent(productId)}`, {
          signal: controller.signal,
          credentials: "include",
        });
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const r = await fetch(`${getApiBase()}/wishlist/check_wishlist_status.php?product_id=${encodeURIComponent(productId)}`, {
          signal: controller.signal,
          credentials: "include",
        });
        if (!r.ok) throw new Error(`HTTP ${r.status}`);
  ...
  const normalized = useMemo(() => {
    ...
    // proxy remote images and /images/ paths through image.php if present
    const photoUrls = photos.map((p) => {
      ...
      return raw.startsWith("/") ? `${getPublicBase()}${raw}` : raw;
    });
  ...
  const handleWishlistToggle = async () => {
    ...
    try {
      const endpoint = isInWishlist 
        ? `${getApiBase()}/wishlist/remove_from_wishlist.php`
        : `${getApiBase()}/wishlist/add_to_wishlist.php`;
  ...
  const handleMessageSeller = async () => {
    ...
    try {
      ...
      const res = await fetch(`${getApiBase()}/chat/ensure_conversation.php`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Accept: "application/json",
